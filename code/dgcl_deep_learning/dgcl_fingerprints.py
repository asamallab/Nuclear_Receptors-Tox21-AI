...
Graph Neural Networks + Molecular Fingerprints for Prediction
...
if len(sys.argv) != 7:
    print(
        "Usage: python3 dgcl_hpc_fingerprints.py "
        "<train_csv> <val_csv> <test_csv> <fp_csv> <checkpoint_dir> <output_txt>"
    )
    sys.exit(1)
...
def smiles_to_graph(smiles, label=None, fp=None):
    ...
    if fp is not None:
        data.fp = torch.tensor(np.array(fp, dtype=np.float32)).unsqueeze(0)
    return data
...
fp_raw      = pd.read_csv(fp_path)
fp_id_col   = fp_raw.columns[0]
fp_raw      = fp_raw.rename(columns={fp_id_col: 'dsstox_substance_id'})
...
# MERGE DATASETS
def merge_and_extract(df, fp_df):
    merged    = pd.merge(df[['dsstox_substance_id', LABEL_COL, 'SMILES']], fp_df, on='dsstox_substance_id', how='inner')
...
train_smiles, train_labels, train_fp, train_ids = merge_and_extract(train_df, fp_raw)
val_smiles,   val_labels,   val_fp, val_ids   = merge_and_extract(val_df,   fp_raw)
test_smiles,  test_labels,  test_fp, test_ids  = merge_and_extract(test_df,  fp_raw)

# FINGERPRINT FILTERING
nan_frac   = train_fp.isna().mean(axis=0)
valid_cols = nan_frac[nan_frac <= 0.20].index.tolist()

train_fp   = train_fp[valid_cols].fillna(train_fp[valid_cols].median())
val_fp     = val_fp[valid_cols].fillna(train_fp[valid_cols].median())
test_fp    = test_fp[valid_cols].fillna(train_fp[valid_cols].median())

selector = VarianceThreshold(threshold=0.0)
selector.fit(train_fp)
train_fp = train_fp.loc[:, selector.get_support()]
val_fp   = val_fp.loc[:, selector.get_support()]
test_fp  = test_fp.loc[:, selector.get_support()]

corr_matrix = train_fp.corr().abs()
upper_tri   = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))
to_drop     = [col for col in upper_tri.columns if any(upper_tri[col] > 0.95)]

train_fp    = train_fp.drop(columns=to_drop)
val_fp      = val_fp.drop(columns=to_drop)
test_fp     = test_fp.drop(columns=to_drop)

FINAL_FP_NAMES   = train_fp.columns.tolist()
NUM_FINGERPRINTS = len(FINAL_FP_NAMES)

train_fp_scaled = train_fp.values.astype(np.float32)
val_fp_scaled   = val_fp.values.astype(np.float32)
test_fp_scaled  = test_fp.values.astype(np.float32)

# BUILD GRAPH DATASETS
def build_dataset(smiles_list, labels, fp_array, id_list):
    dataset, valid_smiles, valid_ids = [], [], []
    for i in range(len(smiles_list)):
        g = smiles_to_graph(smiles_list[i], labels[i], fp_array[i])
        if g is not None:
            dataset.append(g)
...
train_data, train_smiles_valid, train_ids_valid = build_dataset(train_smiles, train_labels, train_fp_scaled, train_ids)
val_data,   val_smiles_valid,   val_ids_valid   = build_dataset(val_smiles,   val_labels,   val_fp_scaled, val_ids)
test_data,  test_smiles_valid,  test_ids_valid  = build_dataset(test_smiles,  test_labels,  test_fp_scaled, test_ids)
...
class ClassifierWithFP(nn.Module):
    def __init__(self, gin_encoder, gat_encoder, num_fingerprints, dropout, graph_dim=128, fp_out_dim=64):
        super().__init__()
        self.gin_encoder = gin_encoder
        self.gat_encoder = gat_encoder
        self.fp_mlp      = FingerprintMLP(input_dim=num_fingerprints, hidden_dim=fp_out_dim, dropout=dropout)
        fused_dim = graph_dim + graph_dim + fp_out_dim
    ...
    def forward(self, data):
        ...
        fp_input = data.fp.view(data.fp.size(0), -1)
        fp_emb   = self.fp_mlp(fp_input)
        fused    = torch.cat([gin_emb, gat_emb, fp_emb], dim=1)
...
for trial_num, combo in enumerate(all_combos):
    ...
    t_start = time.time()
    nw = 0
    t_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=nw)
    v_loader = DataLoader(val_data,   batch_size=batch_size, num_workers=nw)
...
best_pretrain_model = DGCLModel(input_dim=NODE_FEATURE_DIM, edge_dim=EDGE_FEATURE_DIM, dropout=best_params['dropout']).to(device)
best_pretrain_model.load_state_dict(best_pretrain_state)

best_classifier = ClassifierWithFP(
    gin_encoder=best_pretrain_model.gin_encoder, gat_encoder=best_pretrain_model.gat_encoder,
    num_fingerprints=NUM_FINGERPRINTS, dropout=best_params['dropout']
).to(device)
best_classifier.load_state_dict(best_classifier_state)

test_loader  = DataLoader(test_data, batch_size=best_params['batch_size'], num_workers=0)
...
torch.save({
    ...
    'fingerprint_names'    : FINAL_FP_NAMES,
    'test_metrics'         : test_metrics,
}, BEST_MODEL_PT)

with open(output_txt, "w") as f:
    f.write("DGCL GIN+GAT EDC Classification Results (Graph + Fingerprints)\n")
    f.write("="*60 + "\n")
    f.write(f"Training samples:   {len(train_data)}\n")
    f.write(f"Validation samples: {len(val_data)}\n")
    f.write(f"Test samples:       {len(test_data)}\n")
    f.write(f"Fingerprints used:  {NUM_FINGERPRINTS}\n")
    f.write("Filtering: NaN (>20%) + Correlation (>0.95) — no variance filter\n")
    f.write("\n")
