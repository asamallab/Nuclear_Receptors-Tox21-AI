# Fingerprint and Descriptor-Based Classification Model
...
``` bash
python3 model.py <x_train> <y_train> <x_test> <y_test> <features_file> <descriptors_file> <output_file>
```

### Example (Both fingerprint and descriptor data)

``` bash
python3 /path/model.py /path/train_X.csv /path/train_y.csv /path/test_X.csv /path/test_y.csv /path/selected_features.txt /path/descriptors.csv /path/model_name_results
```
